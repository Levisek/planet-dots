...
test('MOONS má přesně 26 měsíců (23 V2 + 3 Neptune)', () => {
...
test('rozložení rodin: Earth 1, Mars 2, Jupiter 6, Saturn 9, Uranus 5, Neptune 3', () => {
  ...
  assert.equal(byParent.jupiter, 6);
...
test('MOONS_BY_PARENT je správně seskupený', () => {
  ...
  assert.equal(MOONS_BY_PARENT.jupiter.length, 6);
  assert.equal(MOONS_BY_PARENT.saturn.length, 9);
});

test('MOON_BY_ID obsahuje všech 26', () => {
  for (const m of MOONS) {
